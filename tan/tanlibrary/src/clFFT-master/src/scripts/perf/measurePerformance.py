...
#problem_size_combinations = iteragents.product(args.lengthx, args.lengthy, args.lengthz, args.batchSize)
#problem_size_combinations = list(iteragents.islice(problem_size_combinations, None))
...
if args.problemsize and args.problemsize[0] != 'None':
    for n in args.problemsize:
        ...
        combos = iteragents.product(x, y, z, batch)
        combos = list(iteragents.islice(combos, None))
        for n in combos:
...
test_combinations = iteragents.product(problem_size_combinations, args.device, args.inputlayout, args.outputlayout, args.placeness, args.precision)
test_combinations = list(iteragents.islice(test_combinations, None))
...
for params in test_combinations:
    ...
    if writeline:
        try:
            if args.library == 'cuFFT':
              output = iteragents.ifilter( lambda x: x.count('gflops'), output)
            else:
              output = iteragents.ifilter( lambda x: x.count('gflops'), output)

            output = list(iteragents.islice(output, None))
